"""
XGBoost Regressor trainer with leak-safe imputation and time-respecting inner validation tuning.
"""
...
class XGBoostModelTrainer:
    def __init__(self, feature_cols: List[str], target_col: str = "next_month_return"):
        ...
        # Expanded grid with lower/more conservative values
    ...
    def train_and_predict(
        ...
        """
        Executes two-stage fit for XGBoost Regressor:
        ...
        for n_est in self.grid_n_estimators:
            for depth in self.grid_max_depth:
                for lr in self.grid_learning_rate:
                    model = XGBRegressor(
                        n_estimators=n_est,
                        max_depth=depth,
                        learning_rate=lr,
